...
export default function AdminDashboardPage() {
     ...
     const setInterview = async (id: string, date?: any, time?: string, link?: string, interviewerEmailsStr?: string, triggerEmail: boolean = false) => {
          if (!date && !link && !time) return;
          if (triggerEmail) setIsSubmitting(true);
          const result = await scheduleInterview(id, date, time, link, interviewerEmailsStr, triggerEmail);
                                                                       ...
                                                                       <Popover>
                                                                            ...
                                                                            <PopoverContent className="w-auto p-0 rounded-3xl border-white/[0.08] shadow-2xl bg-[#09090B]" align="start">
                                                                                 <CalendarUI
                                                                                      ...
                                                                                      onSelect={(date) => {
                                                                                           if (date) setInterview(app._id, date.toISOString(), app.interviewTime, app.interviewLink, app.interviewerEmails?.join(', '), false);
                                                                                      }}
                                                                                      initialFocus
                                                                                      className="p-4"
                                                                                 />
